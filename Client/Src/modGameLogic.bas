...
Public Sub GameLoop()
    ...
    Do While InGame
        ...
        If FpsLowTick < Tick Then
            FpsLowTick = Tick + 10000
            
            If FpsLowCount > 5 Then
                If VerProcess <> 2 Then
                    If MsgBox("Your game's FPS have been consistantly low, try software processing?", vbYesNo) = vbNo Then
                        GameData.VerProcess = VerProcess
                    Else
                        GameData.VerProcess = 2
                        ...
                        Set DX8 = Nothing
                        Set DX8 = New clsDX8
                        Call DX8.InitDirectX(frmMainGame.picScreen.hWnd, D3DCREATE_SOFTWARE_VERTEXPROCESSING)
                    End If
                End If
            End If
    ...
    If Editor = EDITOR_NONE Then
        For x1 = Player(MyIndex).x - 9 To (MAX_MAPX + 1) + Player(MyIndex).x - 8
            For y1 = Player(MyIndex).y - 6 To (MAX_MAPY + 1) + Player(MyIndex).y - 5
                ...
                For n = 0 To 8
                    MapTilePosition(tx, ty).Layer(n).ypos = (map(m).Tile(x, y).Num(n) \ TILESHEET_WIDTH) * PIC_Y
                    MapTilePosition(tx, ty).Layer(n).xpos = (map(m).Tile(x, y).Num(n) Mod TILESHEET_WIDTH) * PIC_X
                Next n
            Next y1
        Next x1
    ElseIf Editor = EDITOR_MAP Then
        For x1 = 0 To MAX_MAPX
            For y1 = 0 To MAX_MAPY
                ...
                For n = 0 To 8
                    MapTilePosition(x, y).Layer(n).ypos = (map(m).Tile(x, y).Num(n) \ TILESHEET_WIDTH) * PIC_Y
                    MapTilePosition(x, y).Layer(n).xpos = (map(m).Tile(x, y).Num(n) Mod TILESHEET_WIDTH) * PIC_X
                Next n
            Next
        Next
    End If
